...
const devToolKey = ((process.platform === 'darwin') ? { // macOS: command+option+i
    alt: true, // option
    control: false,
    meta: true, // command
    shift: false,
...
const createMainWindow = () => {
    ...
    webContents.on('will-prevent-unload', ev => {
        const choice = dialog.showMessageBoxSync(window, {
            ...
            message: formatMessage({
                ...
                default: 'Leave KmEducation?',
                description: 'prompt for leave KmEducation'
            }),
            detail: formatMessage({
                id: 'index.questionLeaveDetail',
                default: 'Any unsaved changes will be lost.',
                description: 'detail prompt for leave KmEducation'
            }),
            buttons: [
                formatMessage({
                    id: 'index.stay',
    ...
    ipcMain.on('reqeustCheckUpdate', () => {
        resourceServer.checkUpdate()
            ...
            .catch(err => {
                webContents.send('setUpdate', { phase: 'error', message: err });
            });
    });

    ipcMain.on('reqeustUpgrade', () => {
        resourceServer.upgrade(state => {
                webContents.send('setUpdate', { phase: state.phase });
            })
            .then(() => {
                app.relaunch();
                app.exit();
